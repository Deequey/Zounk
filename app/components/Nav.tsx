...
export async function Nav() {
  ...
  return (
    <header className="sticky top-0 z-[100] w-full border-b border-zinc-200/50 bg-white/75 backdrop-blur-xl dark:border-zinc-900/50 dark:bg-zinc-950/75 transition-all duration-300">
      ...
      <div className="mx-auto flex w-full items-center justify-between px-6 md:px-16 lg:px-24 2xl:px-40 py-4 2xl:py-6">
        ...
        <Link href="/" className="group flex items-center gap-3 relative z-[110]">
          <div className="flex h-10 w-10 2xl:h-12 2xl:w-12 items-center justify-center rounded-2xl transition-all duration-500 overflow-hidden group-hover:scale-105 group-hover:shadow-[0_0_20px_rgba(29,185,84,0.3)]">
            <img src="/Zounk_Logo.png" alt="Zounk Logo" className="h-12 w-12 2xl:h-14 2xl:w-14 object-contain group-hover:rotate-6 transition-transform duration-500" />
          </div>
          <span className="text-2xl 2xl:text-3xl font-black italic uppercase tracking-tighter text-zinc-900 dark:text-zinc-100 group-hover:text-[#0ad150] transition-colors">
            Zounk.
          </span>
        </Link>

        {/* DESKTOP NAV (Center) */}
        <div className="hidden lg:flex items-center bg-zinc-100/80 dark:bg-zinc-900/80 px-2 py-1.5 2xl:px-4 2xl:py-2.5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 shadow-sm transition-all duration-500 backdrop-blur-md">
          {links.map(({ href, label }) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-white dark:bg-zinc-800/50 rounded-xl opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 -z-0 shadow-sm" />
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4 2xl:gap-8 relative z-[110]">
            ...
            <div className="flex items-center gap-3 2xl:gap-5">
              ...
              <div className="relative group cursor-pointer">
                <img
                  ...
                  alt="Avatar"
                  className="h-10 w-10 2xl:h-14 2xl:w-14 rounded-2xl object-cover border-2 border-transparent group-hover:border-[#0ad150] transition-all duration-500 shadow-lg"
                />
                <div className="absolute -bottom-1 -right-1 w-4 h-4 2xl:w-5 2xl:h-5 bg-[#0ad150] rounded-full border-4 border-white dark:border-zinc-950" />
              </div>
          ...
          <label
            ...
            peer-checked:[&>div:nth-child(1)]:rotate-45 peer-checked:[&>div:nth-child(1)]:translate-y-2
            peer-checked:[&>div:nth-child(2)]:opacity-0
            peer-checked:[&>div:nth-child(3)]:-rotate-45 peer-checked:[&>div:nth-child(3)]:-translate-y-2"
          >
            <div className="h-0.5 w-5 bg-zinc-900 dark:bg-zinc-100 transition-all origin-center" />
            <div className="h-0.5 w-5 bg-zinc-900 dark:bg-zinc-100 transition-all origin-center" />
            <div className="h-0.5 w-5 bg-zinc-900 dark:bg-zinc-100 transition-all origin-center" />
          </label>

          {/* FULL SCREEN MOBILE OVERLAY (Placed as a sibling outside of label) */}
          <div className="fixed inset-0 z-[110] h-screen w-screen translate-x-full bg-zinc-950 p-8 transition-transform duration-500 ease-[cubic-bezier(0.85,0,0.15,1)] peer-checked:translate-x-0 overflow-hidden">
            <div className="flex h-full flex-col justify-center gap-6 2xl:gap-8 max-w-4xl mx-auto">
              <p className="font-mono text-[10px] text-[#0ad150] uppercase tracking-[0.5em] mb-4">Navigation</p>
              {links.map(({ href, label }) => (
                <Link
                  key={href}
                  href={href}
                  className="text-5xl sm:text-7xl font-black italic uppercase tracking-tighter text-white hover:text-[#0ad150] transition-all hover:translate-x-4 duration-300 hover:line-through decoration-[#0ad150] decoration-8"
                >
                  {label}.
                </Link>
              ))}
